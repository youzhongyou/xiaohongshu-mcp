...
func (s *AppServer) handleDownloadUserNotes(ctx context.Context, args DownloadUserNotesArgs) *MCPToolResult {
	...
	result, err := s.xiaohongshuService.DownloadUserNotes(ctx, args.UserID, args.XsecToken, args.WithDetail)
	if err != nil {
		return &MCPToolResult{
			Content: []MCPContent{{
				Type: "text",
				Text: "下载用户笔记失败: " + err.Error(),
			}},
			IsError: true,
		}
	}
	...
	if err != nil {
		return &MCPToolResult{
			Content: []MCPContent{{
				Type: "text",
				Text: fmt.Sprintf("下载完成但序列化失败: %v", err),
			}},
			IsError: true,
		}
	}
